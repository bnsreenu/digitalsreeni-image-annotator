...
| Curation seeds the split | Near-duplicate clusters (#72) are **evidence for the train/val grouping**, not just a report. `CurationController.split_groups`/`refine` folds them into `derive_groups` via `core/dataset_split.merge_groups`, and it is the **only** place that happens — `io_controller.split_inputs(mw)` returns `(names, groups)` together so a call site cannot warn about one grouping and export another. The SAM path splits on `"{index}:{name}"` keys, so clusters must go through `translate_clusters` first and the mapping is handed to the worker as `keyed_groups` rather than re-derived there. **`refine` computes nothing when `embeddings` is empty** — that guard is what makes it safe to call from an export path | An earlier attempt clustered *on demand* from the exporter: a pure-Python O(n²) sweep on the GUI thread, 43 s at 800 images, and it ran even at `val_split=0` where the result was discarded. Untranslated clusters on the SAM path match nothing, silently; a re-derived grouping on the worker silently drops the refinement, so the dialog describes one split and the run performs another. See [ADR-045](docs/09_architecture_decisions.md). |
| Similarity at scale | `core/similarity` is one blocked NumPy pass (`_scan`): every threshold's components **and** each row's nearest neighbour come off a single sweep, and components are built by relabelling, **never** by union-find over a materialised edge list. Block height is derived from n so peak memory is constant. Vectors are `float32` arrays end to end — `CurationController` stores arrays, so `_stack` must never test one for truthiness | 20 000 near-identical frames — the case someone opens this tool to diagnose — have 200 million edges but only n−1 possible merges. `array or []` raises. A BFS with per-seed frontiers looks equivalent and is 4× slower at the top of the slider, where nearly everything is a singleton and each tiny matmul re-reads the whole matrix. |
| Qt-free core | `core/annotation_qc`, `core/disagreement`, `core/similarity`, `core/task_inference`, `core/model_sidecar`, `core/project_io`, `core/annotation_types`, `core/image_size`, `core/mask_filters`, `core/onion`, `core/dataset_split`, `core/qt_diagnostics` and **both `io/` modules** must not import Qt at module level | The CLI (#76) imports them. A stray `from PyQt6 ...` makes headless validation require a display, and works fine on the machine of whoever added it. Enforced by a **subprocess** test in `tests/integration/test_cli.py` — in-process would pass regardless, since the suite already imported PyQt6. See [ADR-041](docs/09_architecture_decisions.md). |
| Qt import failure (#92) | `main.py` order is **torch (ADR-017) → guarded PyQt6 import (ADR-046) → everything else** (AST-gated in `test_smoke.py`). The guard wraps **only** the `PyQt6` import, never `annotator_window`, and routes to `core/qt_diagnostics` — which must never import Qt or `LoadLibrary` a DLL (it reads PE version resources off disk). Its search order models **`PyQt6/__init__.py::find_qt()`**, not the Windows loader: `dirname(sys.executable)` wins outright and then the wheel's Qt is *never registered*; `PATH` **is** walked, but only when the wheel ships no Qt. Severities are `error` (proven) / `suspect` (plausible, unprovable without loading the DLL) / `warning` (fine now, fragile later) — `doctor` fails on the first two. `diagnose(env, qt_failed=True)` (only from the startup guard) additionally runs rules whose evidence counts only after a real failure — currently the MSVC comparison. The `PyQt6` cap in `pyproject.toml` is a **tested ceiling, not the fix** — don't tighten it to `<6.9` | `0xc0000139` on `import PyQt6.QtCore` is a Conda env whose own `Qt6Core.dll` / `msvcp140.dll` shadows the wheel's, not a bad PyQt6 release: CI is green unpinned on 6.11 across the matrix. Swapping the torch/Qt import order re-breaks ADR-017, reproducing only on Windows with torch installed. Wrapping `annotator_window` too would report a missing shapely as a Qt failure. Collapsing `suspect` into `warning` is how the first draft printed two mismatched versions and then claimed they matched. The MSVC rule ran unconditionally at first and failed **every** windows-latest CI leg: stock CPython bundles a vcruntime140 a minor behind System32's, so on its own the gap is ~100 % false positives — hence `qt_failed`. The guard is also dead code if `__init__.py` stops being lazy. See [ADR-046](docs/09_architecture_decisions.md). |

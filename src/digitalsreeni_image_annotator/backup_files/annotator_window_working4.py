...
class ImageAnnotator(QMainWindow):
    ...
    def create_slices(self, image_array, dimensions, image_path):
        ...
        if image_array.ndim == 2:
            ...
            for idx in np.ndindex(tuple(image_array.shape[i] for i in slice_indices)):
                ...
                print(f"Slice {idx} - min: {slice_array.min()}, max: {slice_array.max()}, mean: {slice_array.mean()}")
    ...
    def normalize_array(self, array):
        ...
        if array.dtype == np.uint16:
            ...
            array_normalized = array.astype(np.float32) - array.min()
            array_normalized = array_normalized / (array.max() - array.min())
        elif array.dtype == np.uint8:
            # For 8-bit images, apply contrast stretching
            p2, p98 = np.percentile(array, (2, 98))
            array_normalized = np.clip(array, p2, p98).astype(np.float32)

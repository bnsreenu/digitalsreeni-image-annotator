...
class ImageAnnotator(QMainWindow):
    ...
    def load_tiff(self, image_path):
        with open(image_path, 'rb') as f:
            with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                with TiffFile(mm) as tif:
                    image_array = tif.asarray(out='memmap')
        self.process_multidimensional_image(image_array, image_path)

    # def load_czi(self, image_path):
    #     with CziFile(image_path) as czi:
    #         image_array = czi.asarray(max_workers=4)  # Adjust max_workers as needed
    #     self.process_multidimensional_image(image_array, image_path)
    ...
    def load_czi(self, image_path):
        with open(image_path, 'rb') as f:
            with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                with CziFile(mm) as czi:
                    # Create a memory-mapped array
                    image_array = czi.asarray(out='memmap')
    ...
    def create_slices(self, image_array, dimensions, image_path):
        ...
        print(f"Height index: {height_index}, Width index: {width_index}")
        ...
        for idx in np.ndindex(tuple(image_array.shape[i] for i in slice_indices)):
            ...
            print(f"Slice shape: {slice_array.shape}")
            print(f"Slice dtype: {slice_array.dtype}")
            print(f"Slice min: {slice_array.min()}, max: {slice_array.max()}")
    
            # Normalize the slice
            slice_array = self.normalize_array(slice_array)
            
            slice_array = self.adjust_contrast(slice_array)
    ...
    def normalize_array(self, array):
        if array.dtype == np.uint16:
            # Use a percentile-based normalization to handle potential outliers
            p_low, p_high = np.percentile(array, (1, 99))
            array_normalized = np.clip(array, p_low, p_high)
    ...
    def array_to_qimage(self, array):
        if array.ndim == 2:
            ...
            if array.dtype == np.uint8:
                ...
                array_8bit = self.normalize_array(array)
                return QImage(array_8bit.data, width, height, width, QImage.Format_Grayscale8)
            ...
            if array.dtype == np.uint8:
                ...
                # Convert 16-bit to 8-bit for each channel
                array_8bit = np.stack([self.normalize_array(array[:,:,i]) for i in range(3)], axis=2)
                return QImage(array_8bit.data, width, height, bytes_per_line, QImage.Format_RGB888)
    ...
    def toggle_tool(self):
        ...
        if sender.isChecked():
            ...
            self.image_label.current_tool = "polygon" if sender == self.polygon_button else "rectangle" if sender == self.rectangle_button else "sam_magic_wand"

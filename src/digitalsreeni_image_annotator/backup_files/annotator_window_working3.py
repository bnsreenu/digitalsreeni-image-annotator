...
class ImageAnnotator(QMainWindow):
    ...
    def process_multidimensional_image(self, image_array, image_path):
        ...
        if file_name not in self.image_dimensions:
            if image_array.ndim > 2:
                default_dimensions = ['Z', 'Y', 'X'] if image_array.ndim == 3 else ['T', 'Z', 'Y', 'X']
                ...
                while True:
                    ...
                    if dialog.exec_():
                        ...
                        if 'Y' in dimensions and 'X' in dimensions:
                            self.image_dimensions[file_name] = dimensions
                            break
                        else:
                            QMessageBox.warning(self, "Invalid Dimensions", "You must assign both Y and X dimensions.")
                    else:
                        return
            else:
                self.image_dimensions[file_name] = ['Y', 'X']
        ...
        if self.image_dimensions[file_name]:
            ...
            self.current_image = self.array_to_qimage(self.normalize_array(image_array))
            self.slices = []
            self.slice_list.clear()
    ...
    def create_slices(self, image_array, dimensions, image_path):
        ...
        if image_array.ndim == 2:
            ...
            height_index = dimensions.index('Y')
            width_index = dimensions.index('X')
            slice_indices = [i for i, dim in enumerate(dimensions) if dim not in ['Y', 'X']]
            ...
            for idx in np.ndindex(tuple(image_array.shape[i] for i in slice_indices)):
                ...
                normalized_array = self.normalize_array(slice_array)
                qimage = self.array_to_qimage(normalized_array)
    ...
    def normalize_array(self, array):
        ...
        if array.dtype == np.uint16:
            ...
            return ((array.astype(np.float32) - info.min) / (info.max - info.min) * 255).astype(np.uint8)
    ...
    def array_to_qimage(self, array):
        if array.ndim == 2:
            ...
            if array.dtype == np.uint8:
                ...
                raise ValueError(f"Unsupported dtype {array.dtype} for 2D array")
            ...
            if array.dtype == np.uint8:
                ...
                raise ValueError(f"Unsupported dtype {array.dtype} for 3D array")
